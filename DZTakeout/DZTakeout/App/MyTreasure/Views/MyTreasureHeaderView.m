...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        [self setupSubviews];
        [self resetHeaderViewWithUserModel:[UserHelper userItem]];
    }
    return self;
}
...
- (void)setupSubviews {
    ...
    [self.contentView addSubview:self.redPacketsBtn];
//    [self.contentView addSubview:self.redPacketsValue];
    ...
    [self.redPacketsBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        @strongify(self);
...
-(void)gotoRedPacket{
    if (self.delegate && [self.delegate respondsToSelector:@selector(headerView:touchUpInsideredPacketsButton:)]) {
        [self.delegate headerView:self touchUpInsideredPacketsButton:nil];
    }
}
...
- (UIButton *)redPacketsBtn {
    if (!_redPacketsBtn) {
        ...
        [_redPacketsBtn setTitle:@"查看红包余额" forState:UIControlStateNormal];
        [_redPacketsBtn setTitleColor:COLOR_HEX(0x333333) forState:UIControlStateNormal];
        _redPacketsBtn.titleLabel.font = [UIFont systemFontOfSize:12];
        [_redPacketsBtn addTarget:self action:@selector(gotoRedPacket) forControlEvents:UIControlEventTouchUpInside];
    }
